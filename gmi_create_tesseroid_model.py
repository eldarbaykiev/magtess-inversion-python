...
if CREATE_VIM_MODEL:
	...
	with open('apriori_VIM/hemant_VIS.tess', 'w') as tessfile:
		#keeping order as in reference CRUST1.0 file
		for i in range(n_lat-1, -1, -1):
			for j in range(n_lon):
				sus[k] = vim_distribution[k]/abs((Z_top[i, j] - Z_bot[i, j])/1000.0)
				string = str(X[i, j]-gmi_config.WIDTH/2.0) + ' ' + str(X[i, j]+gmi_config.WIDTH/2.0) + ' ' + str(Y[i, j]-gmi_config.WIDTH/2.0) + ' ' + str(Y[i, j]+gmi_config.WIDTH/2.0) + ' ' + str(Z_top[i, j]) + ' ' +  str(Z_bot[i, j]) + ' 1.0 ' + str(sus[k])
				#print string
				tessfile.write(string + '\n')
				k = k + 1

	np.savetxt('apriori_VIM/' + gmi_config.PROJECT_NAME + '.x0', sus)
...
if oper_system == 'Linux':
	tessutil_magnetize_model_filename = 'tessutil_magnetize_model_linux'


import os
if os.path.isfile(tessutil_magnetize_model_filename) == True:
	os.system('./'+ tessutil_magnetize_model_filename + ' ' + gmi_config.IGRF_COEFF_FILENAME + ' model.tess ' + str(gmi_config.IGRF_DAY) + ' ' + str(gmi_config.IGRF_MONTH) + ' ' + str(gmi_config.IGRF_YEAR) + ' model.magtess')
	if CREATE_VIM_MODEL:
		os.system('./'+ tessutil_magnetize_model_filename + ' ' + gmi_config.IGRF_COEFF_FILENAME + ' apriori_VIM/hemant_VIS.tess ' + str(gmi_config.IGRF_DAY) + ' ' + str(gmi_config.IGRF_MONTH) + ' ' + str(gmi_config.IGRF_YEAR) + ' apriori_VIM/hemant_VIS.magtess')
else:
	print ("CAN NOT FIND " + tessutil_magnetize_model_filename + "  IN THE CURRENT DIRECTORY")
	exit(-1)
